  ...
  const handleApprove = async (benevole) => {
    ...
    try {
      const response = await axios.patch(
        `${import.meta.env.VITE_BACKEND_URL}/benevoles/admin/${benevole.id}`,
        {
          compteValide: true,
        },
        axiosConfig
      ...
      setBenevoles((prevBenevoles) =>
        prevBenevoles.filter((item) => item.id !== benevole.id)
      )
    } catch (error) {
      ...
      <table>
        ...
        <tbody>
          {benevoles.map((benevole) => (
            <tr key={benevole.id}>
              <td onClick={() => handleBenevoleClick(benevole)}>
                {benevole.nom}
              </td>
              <td>
                <button onClick={() => handleApprove(benevole)}>
                  Approuver
                </button>
          ...
          <div className='modal-content'>
            {selectedBenevole && (
              <div>
                ...
                <p>
                  Adresse: {selectedBenevole.ville},{' '}
                  {selectedBenevole.codePostal} <br />{' '}
                  {selectedBenevole.adressePostale}
                </p>
